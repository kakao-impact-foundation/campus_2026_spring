...
type Props = {
  projects: Project[];
  // 주제 카테고리 그룹으로 묶어 보여줄지 (과거 학기는 조직 매핑이 없어 false).
...
export default function FilterSidebar({
  ...
  // 카테고리 없이 보여줄 때의 조직 목록 (시트 등장 순서)
  const orgs = [...new Set(projects.map((p) => p.org))].filter(Boolean);

  // 모바일 접힘 상태 — 첫 화면에서 프로젝트가 보이도록 기본 닫힘.
  const [open, setOpen] = useState(false);

  // 현재 선택 요약 (모바일 토글 라벨)
  const current = selection.kind === "all" ? "전체" : selection.val;
        ...
        {showCategories ? (
          <ul className="mb-[22px] list-none">
            {CATEGORIES.map((cat) => {
              ...
              const catActive =
                selection.kind === "cat" && selection.val === cat;
              return (
          ...
          /* 과거 학기 — 주제 그룹 없이 함께한 사회혁신조직 목록만 */
          <ul className="mb-[22px] list-none">
            {orgs.map((org) => {
              const orgActive =
                selection.kind === "org" && selection.val === org;
              return (
                <li key={org} className="mb-1.5">
                  <button
                    onClick={() => pick({ kind: "org", val: org })}
                    aria-pressed={orgActive}
                    className="flex w-full items-center justify-between gap-2 rounded-[9px] px-2.5 py-[9px] text-left text-[14.5px] font-bold hover:bg-soft aria-pressed:bg-soft2"
                  >
                    <span>{org}</span>
                    <span className="text-xs font-semibold text-muted">
                      {orgCount[org] ?? 0}
